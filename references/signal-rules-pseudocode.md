...
def determine_signal(etf: dict, hs300: dict, delay_flag: bool = False):
    ...
    # ══ 阶段5：动量加速/过热风险（二选一，拥挤度拦截） ════════
    # 判断：RSI在65-75区间 + 赔率>1.5 + 均线多头
    # 前置条件：拥挤度 < 70 才触发动量加速；拥挤度 ≥ 70 时替换为"⚠️过热风险"
    ...
    if rsi_accelerating and reward_high and ma_aligned:
        if crowding_score >= 70:
            return "⚠️过热风险", "拥挤度偏高，动量加速阶段过热，回调风险积累"
...
|| P2 | ⚠️过热风险 | 动量加速条件满足 **AND** 拥挤度≥70 | 高位过热状态，注意波动风险 | 正常触发（延迟时降为状态参考） ||
|| P2 | 🟢动量加速 | RSI 65-75 **AND** 赔率>1.5 **AND** 均线多头 **AND** 拥挤度<70 | 高位风险标记，趋势状态延续 | 延迟时禁止触发（在阶段4延迟拦截中处理） ||
...
| P7 | 🔺扩散度低 | 扩散度<40%（作为辅助标注，不独立输出信号） | 警惕假轮动，成分股广度不足 | — |
| P8 | ↑加速爬升 | 5日排名比20日排名提升≥3位且speed_score≥5（作为辅助标注） | 排名持续改善，信号置信度提升 | — |
| — | ⚪中性 | 不满足以上任何条件 | 无信号 | — |
...
- **计算**：综合短期涨幅分位（近10日涨幅在近90日滚动10日收益率分布中的分位）、成交额分位、乖离率、连续阳线数（0-100分）
- **触发**：
  - ≥70分 → 🔥拥挤，输出「短期交易情绪处于历史高位，波动风险需关注」（不阻断综合得分）
  - 50-69分 → ⚡偏热
  - <50分 → 正常
