...
ETF_DATA = {
    ...
    # ── LWO-25 新增字段 ──────────────────────────────────────
    "sector_breadth": 72.5,           # 相对强弱扩散度（%）
    "rank_velocity": {                 # 排名变化 dict
        "speed_score": 7.5,
        "stability": 1.2,
        "trend": "accelerating",       # accelerating / stable / decelerating
        "improvement": 5               # 5日 vs 20日排名差值
    },
    "crowding_score": 78,             # 拥挤度/过热得分（0-100）
    "confirm_status": {               # 确认窗口状态
        "confirmed": True,
        "days_held": 3,
        "confidence": "高"
    },
...
def determine_signal(etf: dict, hs300: dict, delay_flag: bool = False):
    ...
    # ══ 阶段7：回撤关注（弱信号漏标拦截） ═══════════════════
    rel_20d = etf.get("rel_strength", {}).get("20日", 0) or 0
    rel_5d  = etf.get("rel_strength", {}).get("5日", 0) or 0
    rel_10d = etf.get("rel_strength", {}).get("10日", 0) or 0

    if rel_20d < -10:
        return "⚠️20日显著落后", "20日超额收益低于沪深300 10%以上，轮动明显落后"
    if rel_5d < 0 and rel_10d < 0:
        return "⚠️持续走弱", "5日/10日相对强度同时为负，轮动趋势向下"
    if etf["rsi"] < 40 and not (macd_dead and ma_bear):
...
| P2 | ⚠️过热风险 | 动量加速条件满足 **AND** 拥挤度≥70 | 高位过热状态，注意波动风险 | 正常触发 |
| P2 | 🟢动量加速 | RSI 65-75 **AND** 赔率>1.5 **AND** 均线多头 **AND** 拥挤度<70 | 高位风险标记，趋势状态延续 | 禁止触发 |
| P3 | 🟡趋势增强 | 启动观察 **AND** (20日新高 **OR** RSI上穿60) | 趋势确立，关注延续性（可附确认窗口置信度） | 禁止触发（延迟时阻断，不生成新标签） |
| P4 | 🔴启动观察 | MACD金叉 **AND** RSI>50 **AND** 成交量>1.2倍 | MACD金叉+RSI>50，等待价格确认信号（可附确认窗口置信度） | 禁止触发 |
| P5 | 🔵相对走弱 | RSI<40 **AND** MACD死叉 **AND** 均线空头 | 轮动弱化状态 | 正常触发 |
| P6 | 🟡相对强势 | 5日相对强度>2% | 短线相对强势状态（扩散度<40%时附加🔺标注） | 正常触发 |
| P7 | ⚠️20日显著落后 | 20日相对强度<-10% | 轮动明显落后，需关注持续性 | 正常触发 |
| P8 | ⚠️持续走弱 | 5日AND10日相对强度均为负 | 轮动趋势向下 | 正常触发 |
| P9 | ⚠️弱势 | RSI<40且非机会流失/相对走弱 | 趋势偏弱，关注是否恶化 | 正常触发 |
| — | ⚪中性 | 不满足以上任何条件 | 无信号 | — |
...
| 2026-05-20 | 全程最高 | ⚪高位整固 | ✅ | 无衰竭/机会流失信号，高位整固阶段 |
...
CHECKLIST = {
    ...
    # ── LWO-25 新增 ───────────────────────────────────────
    "breadth_enough":       len(closes) >= 21,     # 扩散度计算最少21根K线
    "rank_velocity_enough":  rank_5d is not None and rank_20d is not None,  # 排名加速需5日+20日排名
    "crowding_score_valid":  crowding_score is not None and 0 <= crowding_score <= 100,
}
```
...
- **计算**：近20日板块日收益率跑赢沪深300的日数占比
- **触发**：扩散度<40%时，在强势信号后附加🔺扩散度低标注
- **降级**：K线不足21根 → 不计算，原信号正常输出

### 6.2 轮动强度排名变化（§4.7）

- **计算**：比较5日 vs 10日 vs 20日相对强弱排名
- **触发**：5日排名比20日排名提升≥3位且speed_score≥5 → ↑加速爬升
- **降级**：排名数据不足 → 输出为空，不阻断信号

### 6.3 拥挤度/过热检测（§4.8）

- **计算**：综合短期涨幅分位、成交额分位、乖离率、连续阳线数（0-100分）
- **短期涨幅分位口径**：最近90日窗口内 rolling 10日收益率分布（当前10日收益率在近90日所有10日收益率中的历史分位）
- **触发**：
  - ≥70分 → 🔥拥挤，输出「短期交易情绪处于高位，波动风险上升」（不阻断综合得分）
  - 50-69分 → ⚡偏热
  - <50分 → 正常
- **特殊**：拥挤度≥70时，动量加速信号被替换为⚠️过热风险
- **降级**：数据不足 → 不输出拥挤度标注，原信号正常输出

### 6.4 持续性验证窗口（§4.9）

- **适用信号**：🔴启动观察、🟡趋势增强
- **逻辑**：信号触发后持续验证1-3个交易日，相对强弱>0且量比>1.0则维持
- **输出格式**：「🔴启动观察（确认中，高）」
- **优先级**：确认窗口不阻断反向信号（死叉/机会流失），遇反向信号直接输出反向信号
