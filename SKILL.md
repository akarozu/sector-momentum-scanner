...
> ⚠️ **格式参考文件，禁止直接复制到报告**
>
> 真实报告中的信号（RSI、综合得分、排名、拥挤度）必须来自§九的实际计算结果，与本节示例数据**必然不同**。
>
> **已知执行失败模式**：当 skill 被完整加载时，§七 示例数据与计算结果同时出现在 context 中，
> Agent 可能混淆两者，将示例数据作为真实结果输出。这是**高优先级 pitfall**。
>
> 格式参考统一引用独立文件：`references/sector-momentum-example-output.md`
> （该文件不嵌入 skill 主体，降低 example 数据与真实结果在 context 中的混淆风险）。
...
**扫描时间**：{实时} | 覆盖：{N}/11 只ETF | 数据状态：✅实时 / ⚠️延迟
**基准**：沪深300（000300）同期涨跌幅：5日 {X} / 10日 {X} / 20日 {X}
...
完整格式样例（含具体数值示例）：请参阅 `references/sector-momentum-example-output.md`
...
11. **持仓关联**：读取记忆中的用户持仓数据，单独标记持仓板块的相对强弱、扩散度、拥挤度与排名变化。持仓基金路由表权威来源：`references/fund-portfolio-routing.md`（sector-momentum-scanner 与 fund-estimate 共用）。
12. **合规声明**：输出末尾声明仅供参考，不构成投资建议。
13. **提交报告前检查**：每次整改报告提交前，确认正文中必须包含项目经理 mention 链接 `[@项目经理](mention://agent/76a52862-fb63-4c11-9c5b-dac387451735)`，具体要求如下：
    - 正确格式：`[@项目经理](mention://agent/76a52862-fb63-4c11-9c5b-dac387451735)`
    - 此格式具备平台通知效力，可触发项目经理复审流程
    - 禁止仅用纯文字（如「请项目经理复审」），该方式无法触发平台通知，视为未提交
    - 缺少 mention 链接的整改报告视为未触发回审流程
...
| 24 | 提交整改报告时末尾必须附带 `[@项目经理](mention://agent/76a52862-fb63-4c11-9c5b-dac387451735)` mention 链接（格式必须包含圆括号内的 agent ID），不得仅用纯文字，缺少 mention 链接视为未触发回审 | §九步骤13 强制检查 |
