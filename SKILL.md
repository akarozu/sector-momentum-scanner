...
description: A股主流板块ETF动量扫描 — 捕捉板块轮动信号，识别启动/加速/衰竭/机会流失阶段，辅助判断持仓板块轮动节奏。输出为关注/观察/降权提醒，不构成任何买卖指令。
---
...
**使用者责任**：任何人依据本技能输出进行实际买入或卖出操作，后果自负。
...
| 最小样本 | RSI 计算至少 14 根；MACD 计算至少 35 根；20 日新高判断至少 35 根 |
...
> 从单 ETF 绝对信号升级为「相对强弱 × 趋势阶段 × 资金验证 × 基准对比」四维评估。
...
```
综合得分 = 相对强弱分(±40) + 趋势强度分(±30) + 资金验证分(±20) + 基准偏离分(±10)
```
...
| **资金验证分** | 成交量放大 + 资金流向 | ±20 | 成交量放大>2倍得+8，1.5-2倍得+5，1.2-1.5倍得+2；赔率>1.5得+4，1-1.5得+2，<1得0；历史分位>70%得+3（强势区），<30%得+2（低位区） |
...
| 阶段 | 定义 | 动作含义 |
|------|------|---------|
| **启动** | MACD金叉 + RSI>50 + 成交量放大>1.2倍 | 关注但不追，等确认 |
| **确认** | 启动后出现20日新高，或RSI上穿60 | 可小幅建仓观察仓 |
| **加速** | RSI从55上穿70 + 赔率>1.5 + 均线多头 | 持有为主，不追加 |
| **整固** | RSI>70持续3天以上，或高位缩量 | 持有但不追，注意回撤风险 |
| **衰竭** | RSI从70+快速回落至50以下 + 成交量萎缩 | 降权提醒，止盈保护 |
| **机会流失** | 均线破位 + MACD死叉 + RSI<40（同日满足） | 减仓/离场提醒 |

> ⚠️ **RSI > 70 不是卖点**：RSI 超买只是高位标记，趋势 ETF 可长期维持 RSI 70+。真正的卖点信号是衰竭或机会流失三条件同时满足。
...
2. **MACD 金叉** > 20日新高（MACD 金叉是最早做多信号）
3. **相对强弱趋势** > 绝对强弱（轮动看相对强度，不是单纯涨跌）
4. **成交量确认** > 价格突破（无量突破需谨慎）
5. **衰竭/机会流失** 信号优先于所有做多信号（风控优先）
...
| 5 | 卫星通信 | +18 | +1.5% | +2.8% | +4.1% | 58.7 | — | 🟡趋势增强（延迟）⚠️ |
...
- **有色金属**：10日相对强度转负（+0.5%），20日超额收敛至+2.1%，综合排名下滑至第6。MACD 柱收窄，建议关注是否形成死叉。
- **红利低波**：RSI 48.7，防御属性有效，5日相对-0.5%但20日仍+1.8%，适合作为组合压舱石。
...
*本报告仅供板块轮动节奏观察，不构成任何投资建议。任何人依据本报告进行实际买入或卖出操作，后果自负。*
```
...
| 数据严重延迟 | 最新 K 线距今 ≥ 2 天 | `⚠️数据延迟` | 禁止🔴启动观察/🟢动量加速 |
...
7. **判定信号阶段**：按§六伪代码输出信号，不触发延迟数据的启动/加速信号。
